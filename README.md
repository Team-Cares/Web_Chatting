...
- **UI**
    - 페이지 : 로고, 로그인, 친구목록, 채팅방목록, 채팅방
- **기능**
    - 로그인, 회원가입, 친구 목록 시각화, 친구(검색, 추가, 상세정보), 추천 친구 추가, 채팅방(검색, 생성, 퇴장), 실시간 채팅
<br>
### 👻최형순

- **기능**
    - 서버 API 제작, socket.io API 제작
<br>
...
- GitHub와 discord를 이용하여 개발상태를 공유하며 작업하였습니다.
- 주 4회씩 회의를 진행하며 작업 순서와 방향성에 대한 고민을 나눴습니다.
...
### [로고화면]
- 서비스 접속 초기화면으로 로그인 페이지로 이동하기 이전 페이지 입니다.
...
### [로그인]
- 이메일 주소와 비밀번호를 입력하면 입력창에서 바로 유효성 검사가 진행되고 통과하지 못한 경우 각 경고 문구가 화면에 띄워지게 됩니다.
- 이메일 주소의 형식이 유효하지 않은 경우 또는 비밀번호가 특수문자, 숫자, 문자들의 조합으로 이루어져 있지 않은 경우에는 경고 문구가 띄워집니다.
- 작성이 완료된 후, 유효성 검사가 통과된 경우 로그인에 성공합니다.

| 로그인 |
|----------|
|![로그인](https://github.com/Dejong1706/MyBlog/assets/75114974/42de5103-a2ee-4c39-a81b-2b423c21936b)|

<br>

### [회원가입]
- 로그인과 같은 방식의 유효성 검사를 통과해야만 회원가입에 성공하게 됩니다.
- 유효성 검사를 통과하지 못할 경우 경고 문구가 발생하게 됩니다.
